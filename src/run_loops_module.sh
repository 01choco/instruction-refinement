...
# inference path 
INPUT_PATH=./results/inference-module-ultrafeedback50-2.jsonl
LOOP_DONE=0
...
for i in 0 1 2; do
    ...
    CUDA_VISIBLE_DEVICES=0,1 python -u src/feedback_armo.py --config-path $CFG_PATH --config-name $CFG_NAME
...
PY
    ...
    CUDA_VISIBLE_DEVICES=0.1 python -u src/_feedback_gen.py --config-path $CFG_PATH --config-name $CFG_NAME
    CUDA_VISIBLE_DEVICES=0,1 python -u src/_refine_gen.py --config-path $CFG_PATH --config-name $CFG_NAME
    CUDA_VISIBLE_DEVICES=0,1 python -u src/_response_gen.py --config-path $CFG_PATH --config-name $CFG_NAME
